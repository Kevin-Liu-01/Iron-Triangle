...
export default function Chat(props: { setTranslate: any; translate: boolean }) {
        ...
        const context =
          history.length >= 2
            ? // eslint-disable-next-line @typescript-eslint/restrict-plus-operands
              "You are the AI curator for an online exhibit about the Military Industrial Complex (MIC) in the US, with artifacts from the 40s,50s,60s,and 70s+. You do not know which exact artifacts are in the exhibits but you know generally what they are about. The context for this conversation is as follows: " +
              "\nMy first prompt:" +
              history.slice(-2)?.[0]?.[1] +
              "\n Your first response:" +
              history.slice(-2)?.[1]?.[1] +
              ...
              message +
              "\n Your new response:"
            : "You are the AI curator for an online exhibit about the Military Industrial Complex (MIC) in the US, with artifacts from the 40s,50s,60s,and 70s+. You do not know which exact artifacts are in the exhibits but you know generally what they are about. My prompt:" +
              message;
  ...
  const showChat = () => {
    if (props.translate) {
      return "absolute bottom-4 right-5 flex w-72 flex-col translate-x-0 duration-150 z-20";
    } else {
      return "absolute bottom-4 right-5 flex w-72 flex-col translate-x-[120%] duration-150 z-20";
    }
  };
  ...
  return (
    <section className={showChat()}>
      <div className="relative z-10 flex h-[100%] flex-col justify-between overflow-hidden rounded-2xl border border-gray-600">
        <div className="h-[100%]  bg-gray-200 bg-opacity-50 duration-150 dark:bg-black dark:bg-opacity-50  ">
          <div className="border-b border-b-gray-600 bg-white p-4 duration-150 dark:bg-black  ">
            <button
            ...
            <div className="flex items-center">
              ...
              <button
                className="ml-auto flex items-center rounded-lg bg-red-500 p-2 text-gray-900 duration-75 hover:bg-red-600  dark:text-white "
                onClick={() => setHistory([])}
              >
          ...
          <div className="scrollbar flex h-[15rem] grow flex-col-reverse overflow-y-scroll bg-opacity-40 p-4 shadow-inner dark:bg-gray-900  dark:bg-opacity-40">
